...
| `associate` | 15分ごと（アイドル時） | 記憶を2〜3引き合わせ、繋がりがあれば結ぶ。1件は思い出しやすさで、残りは長く思い出していない記憶を優先しつつ無作為に |
...
誰も探しにいかない組み合わせの記憶は永遠に出会わない。それを引き合わせるのがこの仕事で、
**「寝ている間に何か考えていた」の実体**はここにある。
1件目（アンカー）は思い出しやすい記憶から重要度で選ぶが、残りの道連れは重要度を見ない。
`last_accessed_at` が古い記憶ほど重みを持つ（温度付きの重み付きサンプリングで、無作為性は残す）。
道連れまで重要度で選ぶと、いつも同じ重要な記憶ばかりが結び付けられて、
地味な記憶は永遠に誰とも引き合わされなくなるため。cold/merged な記憶は対象外。
